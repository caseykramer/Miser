...
type ProvidedTypeDefinition(container:TypeContainer,className : string, baseType  : Type option) as this =
    ...
    override __.Module : Module = typeof<ProvidedTypeDefinition>.Module//notRequired "Module" this.Name
                ...
                let rec emit (expectedState : ExpectedStackState) (expr: Quotations.Expr) = 
                    ...
                    | Quotations.Patterns.Value (obj, _ty) -> 
                        let rec emitC (v:obj) = 
                            ...
                            | :? System.Enum as x when x.GetType().GetEnumUnderlyingType() = typeof<byte> -> ilg.Emit(OpCodes.Ldc_I4, int32 (unbox<byte> v))
#endif
                            | :? Type as ty ->

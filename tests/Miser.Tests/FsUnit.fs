...
//
[<AutoOpen;DebuggerNonUserCodeAttribute>]
module TopLevelOperators =
    ...
    let shouldMsg (f: 'a -> #Constraint) x (y : obj) msg = 
        let c = f x
        let y = 
            match y with
            | :? (unit -> unit) -> box (TestDelegate(y :?> unit -> unit))
    ...
    let shouldMsgf (f: 'a -> #Constraint) x (y: obj) msg= 
        Printf.ksprintf (fun m -> shouldMsg f x y m) msg
